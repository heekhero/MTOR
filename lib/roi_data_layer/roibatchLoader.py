...
from model.utils.rotate_util import *
...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, target=False):
        self._roidb = roidb
        ...
        for i in range(num_batch):
            ...
            self.ratio_list_batch[left_idx:(right_idx + 1)] = torch.tensor(target_ratio,
                                                                           dtype=torch.double)  # trainset ratio list ,each batch is same number
